...
def create_staff(sign_up_body: Operator_Register_Model):
    try:
        user = Operator_Mongo_Document(**sign_up_body.model_dump())
        user.save()
...
def sign_in_staff(sign_in_body: Operator_Login_Model):
    try:
        email=sign_in_body.email
        password=sign_in_body.password
        staff = Operator_Mongo_Document.objects(email=email).first()
        if not staff:
...
def sign_out_staff(sign_out_body: Operator_Logout_Model):
    try:
